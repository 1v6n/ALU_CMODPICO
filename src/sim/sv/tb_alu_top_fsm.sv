...
module tb_alu_top_fsm;
    ...
    task automatic assert_estado(input [7:0] exp_result, input bit exp_cout, input bit exp_zero, input string mensaje);
        begin
            registrar_check(
                $sformatf("%s -> result=0x%0h (exp 0x%0h) Cout=%0b (exp %0b) Zero=%0b (exp %0b)",
                          mensaje, alu_result, exp_result, alu_cout, exp_cout, alu_zero, exp_zero),

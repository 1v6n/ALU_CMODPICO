...
module alu_register_bank #(
    ...
    localparam integer REG_COUNT = 3;  
    localparam integer REG_A_IDX = 0; 
    localparam integer REG_B_IDX = 1;  
    localparam integer REG_SEL_IDX = 2; 
    localparam integer OPCODE_PAD_WIDTH = (DATA_WIDTH > OPCODE_WIDTH) ? DATA_WIDTH - OPCODE_WIDTH : 0; 

    reg [DATA_WIDTH-1:0] register_bank [0:REG_COUNT-1];

    integer bank_idx;  

    assign reg_a = register_bank[REG_A_IDX];
    assign reg_b = register_bank[REG_B_IDX];
    assign reg_sel = register_bank[REG_SEL_IDX][OPCODE_WIDTH-1:0]; 
